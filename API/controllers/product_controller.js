...
const viewProductViaProductName = (req, res, next) => {
    ...
    if (prodName == null || prodName.trim() === "") {
      ...
      let query = `
      SELECT tbl_brand.name AS brand_name , tbl_category.type, tbl_product.name, description, created_at, main_img, left_img, 
      right_img, top_img 
...
const viewOldestToNewest = (req,res,next)=>{        
    ...
    main_img, left_img, right_img, top_img 
    FROM tbl_product 
    JOIN tbl_brand ON tbl_product.brand_id = tbl_brand.id 
    JOIN tbl_category ON tbl_product.category_id = tbl_category.id 
...
const viewLowestToHighest = (req,res,next)=>{        
    let selectQuery = `
    SELECT tbl_brand.name AS brand_name , tbl_category.type, p.name, p.description, p.created_at, 
    p.main_img, p.left_img, p.right_img, p.top_img , min_price.min_price
